...
// Called every frame
void AAITrack::Tick(float DeltaTime)
{
...
void AAITrack::InitCircuitDistance()
{
	float distance = 0.f;
	FVector target[2];
	bool state = true;
...
bool AAITrack::UpdatePoint(const FVector & actorloc, int& index)
{
	FVector target[2];
...
float AAITrack::CalcRectPosition(const FVector & actorloc, const int index)
{
	FVector t[3];
